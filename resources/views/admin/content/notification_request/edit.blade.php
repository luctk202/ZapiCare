...
@endsection
@section('content')
    <form method="POST" action="{{ route('admin.notification.update', ['notification' => $data->id]) }}"
                ...
                <div class="row">
                    ...
                    <div class="col-sm-12 mb-1">
                        ...
                        <select class="form-select" id="type" name="type">
                            <option value="1" @if(old('type', $data->type) == 1) selected @endif>Khuyến mại</option>
                            <option value="2" @if(old('type', $data->type) == 2) selected @endif>Thành viên</option>
                        </select>
                        @error('type')
                        <span class="form-text text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="col-sm-12 mb-1">
                        ...
                        <input
                                type="text"
                                name="time_send"
                                value="{{ old('time_send', date('d-m-Y H:i', $data->time_send)) }}"
