                ...
                <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                <a href="{{ route('admin.product-category.create') }}" class="btn btn-success">Thêm danh mục</a>
                            </div>
                        </div>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(!empty($category->image))
                                            <img src="{{ \Illuminate\Support\Facades\Storage::url($category->image) }}" alt="{{ $category->name }}" width="80" >
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div class="form-check form-check-primary form-switch">
                                            <input type="checkbox" data-id="{{ $category->id }}"
                                                   @if($category->status == \App\Repositories\ProductCategory\ProductCategoryRepository::STATUS_ACTIVE) checked="true"
                                                   @endif class="form-check-input js_update_status"/>
                                        </div>
                                    </td>
                                    <td >
                                        <div class="d-flex align-items-center col-actions">
                                            <a class="me-2" href="{{ route('admin.product-category.edit',['product_category' => $category->id]) }}"
        ...
        var ProSellPage = {
            ...
            updateStatus:function (id, status){
                if (ProSell.config.ajaxProcess === false) {
                    ...
                    Axios({
                        method: 'post',
                        url: '/admin/product-category/update-status/'+id,
                        data:{status:status}
                    }).then(function (response) {
                        let res = response.data;
                        if (res.result === true) {
            ...
            updateHot:function (id, hot){
                if (ProSell.config.ajaxProcess === false) {
                    ...
                    Axios({
                        method: 'post',
                        url: '/admin/product-category/update-hot/'+id,
                        data:{hot:hot}
                    }).then(function (response) {
                        let res = response.data;
                        if (res.result === true) {
            ...
            updateHome:function (id, home){
                if (ProSell.config.ajaxProcess === false) {
                    ...
                    Axios({
                        method: 'post',
                        url: '/admin/product-category/update-home/'+id,
                        data:{home:home}
                    }).then(function (response) {
                        let res = response.data;
                        if (res.result === true) {
