                        ...
                        <table class="table table-bordered">
                            ...
                            @foreach($data as $news)
                                <tr>
                                    <td>
                                        @if(!empty($news->image))
                                            <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($news->image) }}" alt="{{ $news->title }}" width="80" height="80">
                                        @endif
                                    </td>
                                    <td>
                                        {{ $news->title }}
                                    ...
                                    {{--<td>
                                        {{ $news->blog_category->name ?? '' }}
                                    </td>--}}
                                    <td>
                                        <div class="form-check form-check-primary form-switch">
                                            <input type="checkbox" data-id="{{ $news->id }}" @if($news->status == \App\Repositories\Blog\BlogRepository::STATUS_ACTIVE) checked="true" @endif class="form-check-input js_update_status"/>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                ...
                                                <a class="dropdown-item"
                                                   href="{{ route('admin.blogs.edit',['blog' => $news->id]) }}">
        ...
        var ProSellPage = {
            ...
            updateStatus:function (id, status){
                if (ProSell.config.ajaxProcess === false) {
                    ...
                    Axios({
                        method: 'post',
                        url: '/admin/blogs/update-status/'+id,
                        data:{status:status}
                    }).then(function (response) {
                        let res = response.data;
                        if (res.result === true) {
