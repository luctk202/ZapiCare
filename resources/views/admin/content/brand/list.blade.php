                ...
                <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                <a href="{{ route('admin.brand.create') }}" class="btn btn-success">Thêm thương hiệu</a>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-bordered">
                            ...
                            @foreach($data as $dt)
                                <tr>
                                    ...
                                    <td>
                                        @if(!empty($dt->logo))
                                            <img src="{{ \Illuminate\Support\Facades\Storage::url($dt->logo) }}" alt="{{ $dt->name }}" width="80" >
                                        @endif
                                    </td>
                                    {{--<td>
                                        @if(!empty($dt->image))
                                            <img src="{{ \Illuminate\Support\Facades\Storage::url($dt->image) }}" alt="{{ $dt->name }}" width="80" >
                                        @endif
                                    </td>--}}
                                    <td>
                                        <div class="form-check form-check-primary form-switch">
                                            <input type="checkbox" data-id="{{ $dt->id }}"
                                                   @if($dt->status == \App\Repositories\Brand\BrandRepository::STATUS_SHOW) checked="true"
                                    ...
                                    {{--<td>
                                        <div class="form-check form-check-primary form-switch">
                                            <input type="checkbox" data-id="{{ $dt->id }}"
                                                   @if($dt->hot == \App\Repositories\Brand\BrandRepository::HOT) checked="true"
                                                   @endif class="form-check-input js_update_hot"/>
                                        </div>
                                    </td>--}}
                                    <td>
                                        {{ $dt->created_at }}
                                    ...
                                    <td >
                                        <div class="d-flex align-items-center col-actions">
                                            <a class="me-2" href="{{ route('admin.brand.edit',['brand' => $dt->id]) }}"
        ...
        var ProSellPage = {
            ...
            updateStatus:function (id, status){
                if (ProSell.config.ajaxProcess === false) {
                    ...
                    Axios({
                        method: 'post',
                        url: '/admin/brand/update-status/'+id,
                        data:{status:status}
                    }).then(function (response) {
                        let res = response.data;
                        if (res.result === true) {
            ...
            updateHot:function (id, hot){
                if (ProSell.config.ajaxProcess === false) {
                    ProSell.loading.show()
                    ProSell.config.ajaxProcess = true
                    Axios({
                        method: 'post',
                        url: '/admin/brand/update-hot/'+id,
                        data:{hot:hot}
                    }).then(function (response) {
                        let res = response.data;
                        if (res.result === true) {
