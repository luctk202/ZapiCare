...
@section('content')
    <form method="POST" action="{{ route('admin.sample-data.update', ['sample_data' => $data->id]) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-3 col-md-6">
                                <label for="test_item_id">Tên mục kiểm tra <span class="text-danger">*</span></label>
                                <select class="form-select select2s" name="test_item_id" id="test_item_id">
                                    <option value="0">Vui lòng chọn</option>
                                    @foreach ($test_items as $test_item)
                                        <option value="{{ $test_item->id }}" {{ $test_item->id == $data->test_item_id ? 'selected' : '' }}>
                                            {{ $test_item->prefix . $test_item->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('test_item_id')
                            ...
                            <!-- Cấp độ -->
{{--                            <div class="col-lg-3 col-md-6">--}}
{{--                                <label class="col-form-label" for="level_name">Cấp độ <span class="text-danger">*</span></label>--}}
{{--                                <input type="text" class="form-control" id="level_name" value="{{ $data->level->name }}" readonly>--}}
{{--                                <input type="hidden" name="level_id" value="{{ $data->level->id }}">--}}
{{--                            </div>--}}

                            <!-- Khoảng min -->
{{--                            <div class="col-lg-2 col-md-6">--}}
{{--                                <label class="col-form-label" for="range_min">Khoảng min</label>--}}
{{--                                <input type="text" id="range_min" class="form-control" name="range_min" value="{{ old('range_min', $data->range_min) }}">--}}
{{--                                @error('range_min')--}}
...
{{--                            <!-- Khoảng max -->--}}
{{--                            <div class="col-lg-2 col-md-6">--}}
{{--                                <label class="col-form-label" for="range_max">Khoảng max</label>--}}
{{--                                <input type="text" id="range_max" class="form-control" name="range_max" value="{{ old('range_max', $data->range_max) }}">--}}
{{--                                @error('range_max')--}}
{{--                                <span class="form-text text-danger">{{ $message }}</span>--}}
{{--                                @enderror--}}
{{--                            </div>--}}
                        ...
                        <!-- Lý giải chỉ số -->
                        <div class="mb-3">
                            <label class="col-form-label" for="explanation">Lý giải chỉ số</label>
                            <textarea id="explanation" class="form-control" name="explanation">{{ old('explanation', $data->explanation) }}</textarea>
                            @error('explanation')
                            <span class="form-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <!-- Dấu hiệu thường gặp -->
                        <div class="mb-3">
                            <label class="col-form-label" for="symptom">Dấu hiệu thường gặp</label>
                            <textarea id="symptom" class="form-control" name="symptom">{{ old('symptom', $data->symptom) }}</textarea>
                            @error('symptom')
                            <span class="form-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <!-- Bệnh lý thường gặp -->
{{--                        <div class="mb-3">--}}
{{--                            <label class="col-form-label" for="disease">Bệnh lý thường gặp</label>--}}
{{--                            <textarea id="disease" class="form-control" name="disease">{{ old('disease', $data->disease) }}</textarea>--}}
{{--                            @error('disease')--}}
{{--                            <span class="form-text text-danger">{{ $message }}</span>--}}
{{--                            @enderror--}}
{{--                        </div>--}}
                        <div class="mb-3">
                            <label class="col-form-label" for="product_id">Bệnh lý thường gặp <span class="text-danger">*</span></label>
                            <select class="form-select select2s" name="disease_ids[]" id="disease_id" multiple>
                                @foreach ($diseases as $value)
                                    <option value="{{ $value->id }}"
                                            @if (is_array(old('disease_ids', $data->diseases->pluck('id')->toArray())) && in_array($value->id, old('disease_ids', $data->diseases->pluck('id')->toArray())))
                                                selected
                                        @endif>
                                        {{$value->name }}
                                    </option>
                        ...
                        <!-- Tư vấn -->
                        <div class="mb-3">
                            <label class="col-form-label" for="advice">Tư vấn</label>
                            <textarea id="advice" class="form-control" name="advice">{{ old('advice', $data->advice) }}</textarea>
                            @error('advice')
                            <span class="form-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
