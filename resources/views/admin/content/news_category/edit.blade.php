...
@section('content')
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.news-category.update',['news_category' => $data->id]) }}"  enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="col-12">
                                ...
                                <div class="d-flex overflow-scroll" id="preview_image">
                                    <div style="width: 110px;height: 160px;border: 1px solid #ccc;margin:5px;position: relative">
                                        <div style="width: 110px;height: 110px;padding:5px">
                                            <image style="object-fit: contain;max-height: 100%;max-width: 100%" alt="{{ $data->name }}" src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($data->image) }}"/>
                                        </div>
