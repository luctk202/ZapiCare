      ...
      <li class="nav-item me-auto">
        <a class="navbar-brand" href="{{ url('/') }}">
          {{--<span class="brand-logo">
            <img src="{{ asset('images/logo/logo-bellhome.png') }}" alt="" >
          </span>--}}
          <h2 class="brand-text" style="margin-left: 56px;margin-top: 18px">ZapiCare</h2>
        </a>
      </li>
      ...
      @if (isset($menuData[0]))
        @foreach ($menuData[0]->menu as $menu)
          ...
          @else
            ...
            @endphp
            <li
              class="nav-item {{ $custom_classes }} {{ in_array(Route::currentRouteName(), $menu->slug) ? 'active' : '' }}">
              <a href="{{ isset($menu->url) ? url($menu->url) : 'javascript:void(0)' }}" class="d-flex align-items-center"
                target="{{ isset($menu->newTab) ? '_blank' : '_self' }}">
                <i data-feather="{{ $menu->icon }}"></i>
                <span class="menu-title text-truncate">{{ translate($menu->name) }}</span>
                @if (isset($menu->badge))
